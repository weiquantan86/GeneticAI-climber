...
#generate creature from the elite class
cr = creature.Creature(gene_count=3)
dna = genome.Genome.from_csv('9_elite.csv')
cr.update_dna(dna)
    ...
    if step % 120 == 0:
        ...
        for jid in range (p.getNumJoints(rob1)):
            ...
            p.setJointMotorControl2(rob1,
                                    ...
                                    controlMode = mode,
                                    targetVelocity = vel)
            #remove this code below to stop the camera
            new_pos,orn = p.getBasePositionAndOrientation(rob1)
            p.resetDebugVisualizerCamera(10,0,200,new_pos)
            #remove the code above to stop the camera
